...
const jobs = [
  {
    ...
    cron: "Horario: todos los días 03:00"
  },
  {
  ...
  },
  {
    ...
    cron: "Horario: todos los días 23:00"
  }
] as const;
      ...
      <section className="panel p-5">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          {jobs.map((job) => {
            ...
            return (
              <div className="panel grid grid-cols-[1fr_34px] gap-3 p-4" key={job.path}>
                <span className="flex min-w-0 gap-4 text-left">
                ...
                <button className="btn h-8 min-h-8 w-8 self-start px-0" disabled={running === job.path} onClick={() => runJob(job.path, job.title)} title={`Ejecutar ${job.title}`} type="button">
